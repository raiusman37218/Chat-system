...
async function runTests() {
  ...
  const agentEmail = 'agent@chatify.io';
  const agentPassword = 'ChatifyDemo2026!';
  ...
  if (signErr) {
    console.log('Notice during sign up:', signErr.message);
  } else {
    console.log('✓ Agent signed up via Supabase Auth API');
  }
  ...
  if (!agentProfile) {
    const { data: newProfile } = await supabase.from('agents').insert({
      id: authData.user.id,
      name: 'Alex Morgan',
      email: agentEmail,
      status: 'online'
  ...
  console.log(`✓ Agent profile verified: ${agentProfile.name}, Status: ${agentProfile.status}`);
  ...
  // Agent updates status to open and assigns self
